...
module.exports = {
  ...
  getFiles: function (folder, debug = 0) {
    ...
    files = files.filter(
      a => a !== 'README.txt' && a !== '.tmp.drivedownload' && a !== '.tmp.driveupload' && !fs.lstatSync(folder + a).isDirectory()
    );
  ...
  isObjHaveRolesAndUsersArrays(obj, guildId) {
    return (
      Object.hasOwn(obj, guildId) &&
      Object.hasOwn(obj[guildId], 'goodUserRoleId') &&
      Object.hasOwn(obj[guildId], 'users') &&
      Object.hasOwn(obj[guildId], 'afkUserRoleId')
    );
  },
