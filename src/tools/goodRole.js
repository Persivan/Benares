          ...
          if (index === -1) {
            ...
            if (notExistMeansAfk) {
              member.roles
                .add(roleBad)
                .then(() =>
                  console.log('[roles] afkRoflRole Чела нет в бд. Роль успешно добавлена: ', member.user.id, ' ник: ', member.user.globalName)
                )
                .catch(() =>
                  console.log('[roles] afkRoflRole Чела нет в бд. Ошибка при добавление: ', member.user.id, ' ник: ', member.user.globalName)
                );
            }
            return;
          }
